...
@Component({
  selector: 'app-platform-mobile-masters',
  standalone: true,
  imports: [
    FormsModule,
    ReactiveFormsModule,
  ...
  templateUrl: './platform-mobile-masters.component.html',
})
export class PlatformMobileMastersComponent implements OnInit {
    ...
    this.platform
      ...
      .subscribe({
        next: (result) => {
          ...
          if (current) {
            const refreshed = result.items.find((b) => b.id === current.id) ?? null;
  ...
  loadModels(brandId: string): void {
    this.platform.getMobileModels(brandId).subscribe({
      next: (models) => {
  ...
  loadVariants(modelId: string): void {
    this.platform.getMobileVariants(modelId).subscribe({
  ...
  createBrand(): void {
    ...
    this.platform.createMobileBrand({ name: this.brandForm.controls.name.value }).subscribe({
      next: () => {
  ...
  createModel(): void {
    ...
    this.saving.set(true);
    this.platform.createMobileModel(brand.id, { name: this.modelForm.controls.name.value }).subscribe({
      next: () => {
        this.saving.set(false);
  ...
  createVariant(): void {
    ...
    this.saving.set(true);
    this.platform
      .createMobileVariant(model.id, { name: this.variantForm.controls.name.value })
      .subscribe({
        next: () => {
          this.saving.set(false);
  ...
  saveEdit(modal: NgbModalRef): void {
    ...
    if (target.type === 'brand') {
      this.platform.updateMobileBrand(target.item.id, raw).subscribe({
        next: () => {
          done();
          this.loadBrands();
        },
    ...
    if (target.type === 'model') {
      this.platform.updateMobileModel(target.item.id, raw).subscribe({
        next: () => {
          done();
          const brand = this.selectedBrand();
          if (brand) {
    ...
    this.platform.updateMobileVariant(target.item.id, raw).subscribe({
      next: () => {
        done();
        const model = this.selectedModel();
        if (model) {
  ...
  deactivate(target: EditTarget): void {
    ...
    ref.result.then((confirmed) => {
      ...
      const request =
        target.type === 'brand'
          ? this.platform.deactivateMobileBrand(target.item.id)
          : target.type === 'model'
            ? this.platform.deactivateMobileModel(target.item.id)
            : this.platform.deactivateMobileVariant(target.item.id);
